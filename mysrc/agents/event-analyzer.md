...
- If events are from NLO, ensure histograms are weighted by `event.weight` — 
  ...
  MA5's own histogram/cut-flow errors already account for weighted fills;
  if you additionally need a hand-computed statistical error bar from a raw
  event count (rather than MA5's own output), use the effective sample size
  Neff = N * (1 - 2*f_minus)^2 in place of N — see the pheno-analyzer agent
