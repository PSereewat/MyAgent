...
| Scale/PDF uncertainty | User wants μ_R/μ_F scale and/or PDF uncertainty on the cross section from a single run | Set `reweight_scale`/`rw_rscale`/`rw_fscale`/`reweight_PDF` in `--commands` (State 2). Printed envelope summary requires `nevents >= 10000` — see [references/optional_features.md](references/optional_features.md) |
...
### Scale/PDF uncertainty (systematics) from a single run

Use this when the user wants the μ_R/μ_F scale envelope and/or PDF uncertainty on the cross section without a separate rerun. **Requires `nevents >= 10000`** to get the printed envelope summary — see [references/optional_features.md](references/optional_features.md) for the reason and for what to do at lower `nevents`.
...
Output: `Events/run_01/summary.txt` contains a "Scale variation" and "PDF variation" block alongside the plain cross section; `Events/run_01/events.lhe.gz` carries the per-event weights (27 scale + 101 PDF = 128 `<wgt>` entries/event for these settings).
