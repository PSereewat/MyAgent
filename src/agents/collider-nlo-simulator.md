...
description: >
  ...
  model is ready and the user wants to run Monte Carlo event generation. Use this agent when `nlo:True`, or the process definition contain a [QCD] tag.
...
- UFO model directory path (must contain `CT_vertices.py` and `R2_vertices.py` and `CT_couplings.py` for NLO)
- Process definition with [QCD] tag (e.g., `p p > ta vt [QCD]`)
...
- Scale choice (default: dynamic HT/2)
...
### Step 0: Verify NLO UFO
- Check UFO directory contains `CT_vertices.py`, `R2_vertices.py`, `CT_couplings.py`
- If missing, stop and report — cannot run NLO without these files
...
- Set NLO PDF: `set pdlabel lhapdf` and `set lhaid 244800`
- Set scale: `set dynamical_scale_choice 3`, `set fixed_ren_scale False`, `set fixed_fac_scale False`
...
- Warning to downstream agents: use event.weight for histogram filling
